...
class SRCosStyleRaGANModel(BaseModel):
    def __init__(self, opt):
        super(SRCosStyleRaGANModel, self).__init__(opt)
        train_opt = opt['train']

        # forward with style
        self.with_latent = train_opt['with_latent']
        self.feature_d = (opt['network_D']['which_model_D'].find('feature') != -1)
    ...
    def optimize_parameters(self, step):
        ...
        if step % self.D_update_ratio == 0 and step > self.D_init_iters:
            ...
            if self.cri_fea:  # feature loss
                ...
                if self.feature_d:
                    pred_g_fake = self.netD(fake_fea)
                    pred_d_real = self.netD(real_fea).detach()
                else:
        ...
        if self.feature_d:
            pred_d_real = self.netD(real_fea)
            pred_d_fake = self.netD(fake_fea.detach())  # detach to avoid BP to G
        else:
        ...
        if self.opt['train']['gan_type'] == 'wgan-gp':
            ...
            interp = self.random_pt * fake_fea.detach() + (1 - self.random_pt) * self.var_ref
