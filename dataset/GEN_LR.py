...
def reShape(pic, size_):
    y, x, c = pic.shape
    begin_x = x // 2 - size_[0] // 2
    begin_y = y // 2 - size_[1] // 2
    end_x = begin_x + size_[0]
    end_y = begin_y + size_[1]
    res = pic[begin_y:end_y, begin_x:end_x]
    return res

def downSample(in_file, size_ = None):
    pic = cv2.imread(in_file)
    if pic is None:
        return None, None
    if size_ != None:
        pic = reShape(pic, size_)
    # pic = cv2.resize(img, (224, 224), interpolation = cv2.INTER_CUBIC)
    img = pic * 1.0 / 255
    ...
    rlt = (rlt * 255).round() / 255
    return img, rlt

def saveImage(rlt, out_file):
    torchvision.utils.save_image(
            rlt, out_file , nrow = 1, padding = 0, normalize = False)
